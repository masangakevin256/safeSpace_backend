...
export async function addNotification(sender_id, sender_role, recipient_id, recipient_role, type, title, message) {
    try {
        const notification = await pool.query(
            `
            INSERT INTO notifications (sender_id, sender_role, recipient_id, recipient_role, type, title, message, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW()) RETURNING notification_id AS id, *;
            `,
    ...
    try {
        ...
        if (user.roles === "admin") {
            notifications = await pool.query(
                `
                SELECT notification_id AS id, * FROM notifications
                `
            )
        } else if (user.roles === "counselor") {
            notifications = await pool.query(
                `
                SELECT notification_id AS id, * FROM notifications WHERE recipient_id = $1
                `,
                [user_id]
            )
